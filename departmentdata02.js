...
const Omni = [departments,employeeDepartment,employees,employeeSalaries]
//combines all Datasets into one giant array 


function DepData02(array,num) {
    
    const dep = array[0], empDep = array[1], emp = array[2], empSal = array[3]
    // creates local variables with easier names to type 

    let 
    depID = [], //Isolate department ID numbers
    empID = [] // Isolates employee ID numbers
       ...
       for (let i = 0; i < dep.length; i++) {
        ...
        }// pushes all Department ID numbers to depID

        for (let j = 0; j < empDep.length; j++) {
            
            
          empID.push(emp[j][0])
         
        }// pushes all employee ID numbers to empID

    switch (num) {

  //switch takes in the challenge number as it's parameter
...
case 1:
    ...
    function gender() {
      ...
      console.log(`\n------------\n #${num} Employee Gender\n------------\n`);
      console.log(`There are a total of ${m.length} males &  ${f.length} females in this company\n--------------`);
...
case 2:
      ...
      function people(val) {
        ...
        console.log(`There are a total of ${depQ.length} in the ${depN} Departmnet; ID#: ${val}\n--------------`);
        
      }
      console.log(`\n------------\n #${num} Department Employees\n------------\n`);
...
//val is the parameter for the department ID number
      function genderDep(val) {
       
    
        let depQ = [], // depQ array contains all of the  sub arrays that match the deparment ID number  which is assigned by 'val' i.e  [10001,"d005","1986-06-26","9999-01-01"]
        depN ='', // depN gets assigned the Department Namem
        m = [], //  m array contains all male employee sub arrays that match employee ID numbers from em array i.e   [10001,"1953-09-02","Georgi","Facello","M","1986-06-26"]
        f = [], // f array contains all female employee sub arrays that match employee ID numbers from em array i.e   [10002,"1964-06-02","Bezalel","Simmel","F","1985-11-21"]
        em = [] // em array contains all the employee ID numbers that have Department ID numbers that match the department ID assaigned by 'val' i.e [10001]
        ...
        }// Loops through the employeedepartemnt array and finds department IDs that match 'val' then pushes those onto the depQ array
        ...
        }// Loops through the department array to find a department ID number that matches 'val' then assigns the second element of the sub array to depN

        for (let i = 0; i < depQ.length; i++) {
         
          em.push(depQ[i][0])
          
        }// filters the depQ array to just employee ID numbers by looping through the depQ array and pushing the first element of the sub array onto the em array
      
        for (let i = 0; i < em.length; i++) {
          
          for (let j = 0; j < emp.length; j++) {
           
            if (em[i] == emp[j][0] && emp[j][4] == "M") {
              
              m.push(emp[j])

            } else if (em[i] == emp[j][0] && emp[j][4] == "F" ) {
        ...
        }// Uses a nested for loop to compare the em array employee ID numbers to the employee array ID and then compares the employee gender to a string matchign the gender. from there it pushes the employee subarray to its respected gender.

          
        console.log(`There are a total of ${m.length} males & ${f.length} females in the ${depN} Departmnet\n--------------`);
        //gender .length will get the amount of that gender
      }

      console.log(`\n------------\n #${num} Department Genders\n------------\n`);
        
       for (let i = 0; i < dep.length; i++) {
       
        genderDep(dep[i][0])
         
       }// loops through the department ID numbers to output the genders for each division
...
case 4:
      ...
      function depEmpSal(val) {
            
          let depQ = [],  depN = '', eID = [], h = 0, payed = []

          for (let i = 0; i < empDep.length; i++) {
            
            if(val == empDep[i][1] && empDep[i][3] == "9999-01-01"){
          ...
          function sal(x) {
            ...
            for (let i = 0; i < employeeSalaries.length; i++) {
              ...
              if(x == employeeSalaries[i][0]){

                emSal.push(employeeSalaries[i][1])
              }
            
            }

            let highest = emSal[ emSal.length - 1 ]
          ...
          console.log(`The ${depN} Department has a total salary of $${h.toLocaleString()} with ${payed.length} out of ${depQ.length} employees on the payroll\n--------------`);

      }

      console.log(`\n------------\n #${num} Department Total Salaries\n------------\n`);
      for (let i = 0; i < dep.length; i++) {
...
*/
// BONUS: log the salary for each department broken down by gender

case 5:

      function depEmpSalGender(val) {

        let depQ = [],  depN = '', eID = [], h = 0, payed = [],payedM = [],m = 0, payedF = [], f = 0


        //gets all the employees that work in said department 
        for (let i = 0; i < empDep.length; i++) {
        ...
        function sal(x) {
          ...
          for (let i = 0; i < employeeSalaries.length; i++) {
            ...
            if (x == employeeSalaries[i][0] && (employeeSalaries[i+1] != undefined && employeeSalaries[i+1][0] != x || i == employeeSalaries.length - 1) ) {
              highestSAL = employeeSalaries[i][1]
            }
          }
  
          let highest = emSal[ emSal.length - 1 ]
          
          // if ( highestSAL != highest ) {
        ...
        //sorts gender and gets total salary for department
        for (let i = 0; i< eID.length; i++) {
          ...
          if(sal(eID[i]) != undefined){
            ...
            for (let j = 0; j < emp.length; j++) {
                ...
                if (eID[i] == emp[j][0] && emp[j][4] == "M") {

                  payedM.push(eID[i])
            
          } else if(eID[i] == emp[j][0] && emp[j][4] == "F"){
        ...
        //gets male salaries
        for (let i = 0; i < payedM.length; i++) {
         
          if (sal(payedM[i]) != undefined) {
        ...
        //gets female salaries
        for (let i = 0; i < payedF.length; i++) {
         
          if (sal(payedF[i]) != undefined) {
        ...
        //console.log(f);
        console.log(`The ${depN} Department has ${payedM.length} males getting paid a total of $${m.toLocaleString()} \n& ${payedF.length} females getting paid a total of $${f.toLocaleString()} for a total of $${h.toLocaleString()}\n--------------- `);
        
      }

      console.log(`\n------------\n #${num} Gender Department Salaries\n------------\n`);
       for (let i = 0; i < dep.length; i++) {
         
        depEmpSalGender(dep[i][0])
...
*/
// BONUS: log the names of all former employees and their last date of work
...
case 6:
      ...
      function formerName() {
        ...
        for (let i = 0; i < empDep.length; i++) {
          ...
          if (empDep[i][3] != "9999-01-01") {
            
            formerID.push(empDep[i][0])
            former.push(empDep[i])
      ...
      console.log(`\n------------\n #${num} Former Department Employees\n------------\n`);
      
        for (let i = 0; i < former.length; i++) {
          
          for (let j = 0; j < emp.length; j++) {
            
            if(former[i][0] == emp[j][0]){

            console.log(`${emp[j][2]} ${emp[j][3]} last worked in the ${department(former[i][1])} Department in ${emp[j][5]}\n--------------`);
...
*/
// BONUS:  Log all employee names, who have received raises

case 7:

      function salIncrease() {
        
        let eID = empID.slice(0,35), raised = []

      function sal(id) {

        let raises = []

        for (let i = 0; i < empSal.length; i++) {
          
          if (id == empSal[i][0]){

            raises.push(empSal[i][1])
    ...
    console.log(`\n------------\n #${num} EMPLOYEES WHO RECEIVED RAISES\n------------\n`);
    
    for (let i = 0; i < raised.length; i++) {
      
      console.log(`${i+1}) ${raised[i][2]} ${raised[i][3]}\n----------`);
...
default:
      ...
      return 'Please enter a Challenge number 1 - 7'
...
break;
    ...
    //console.log(depID,empID);
}//function with 2 inputs: array,  challenge number
