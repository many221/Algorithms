...
/*
  ...
  +-+-+-+-+-+ +-+-+ +-+ +-+-+-+-+-+-+ 
  |T|o|t|a|l| |o|f| |4| |A|r|r|a|y|s| 
  +-+-+-+-+-+ +-+-+ +-+ +-+-+-+-+-+-+ 
 

 
  +-+-+ +-+-+-+-+-+-+-+-+-+-+-+
  |1|)| |D|e|p|a|r|t|m|e|n|t|s|
  +-+-+ +-+-+-+-+-+-+-+-+-+-+-+
  -_-_-["d001", "Marketing"]-_-_-

    Index 0 = department ID #

    Index 1 = departmant name

    Assigns ID number to Department Names

 
   +-+-+ +-+-+-+-+-+-+-+-+ +-+-+-+-+-+-+-+-+-+-+-+ 
   |2|)| |E|m|p|l|o|y|e|e| |D|e|p|a|r|t|m|e|n|t|s| 
   +-+-+ +-+-+-+-+-+-+-+-+ +-+-+-+-+-+-+-+-+-+-+-+ 
   -_-_- [10001,"d005","1986-06-26","9999-01-01"]-_-_-

    Index 0 = Employee ID #

    Index 1 = Department ID # 

    Index 2 = Employee Department Start Date

    Index 3 = Employee Department End Date (termination or department exchange)("9999-01-01" represents currently employed)

    Assigns Start and End Dates in Departments to employees
   ...
   |3|)| |E|m|p|l|o|y|e|e|s|
   +-+-+ +-+-+-+-+-+-+-+-+-+  
   -_-_- [10001,"1953-09-02","Georgi","Facello","M","1986-06-26"]-_-_-

    Index 0 = Employee ID #

    Index 1 = Employee Birthdate (can calculate employee age based of of time(termination/current - start date))
    ...
    Index 5 = Employement Start Date

  
  +-+-+ +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
  |4|)| |E|m|p|l|o|y|e|e|s|S|a|l|a|r|i|e|s|
  +-+-+ +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    -_-_- [10001,60117,"1986-06-26","1987-06-26"]-_-_-

    Index 0 = Employee ID #

    Index 1 = Salary (The first occurrence of Salary is the employees' starting pay, all subsequent occurrences are either raises or deductions)

    Index 2 =  Salary Start Date (This can line up with employee department exchaganges, First occurrence is employees start date )

    Index 3 = Salary End Date (This can line up with employee department exchaganges, Last occurrence is either current pay or termination date)


 
  +-+-+-+-+-+-+-+ +-+-+-+-+-+-+ 
  |B|r|i|d|g|e|d| |A|r|r|a|y|s| 
  +-+-+-+-+-+-+-+ +-+-+-+-+-+-+ 

    Index 0 = arr2 arr3 arr4


    Dates are string and have - in them
...
let bus = [departments, employeeDepartment, employees,employeeSalaries]

function looperElements(arr,num) {
...
//employee ID isolation for loop
for (let i = 0; i < bus[2].length; i++) {
  
  eID.push(bus[2][i][0])

}

//cuts down to only 40 employees
let eID40 = eID.slice(0,40)
 
//console.log(eID40);

// console.log(

//   // this checks true or false bus[2][i].indexOf(eID40[i])

// );

//will loop through ID numbers and match them with array elements then logs it 
function emSearch(array,ID) {

  let dpNamS = array[0],dpNumS = array[1],namS = array[2]
  ...
  for (let i = 0; i < dpNamS.length; i++) {
   
    for (let j = 0; j < dpNumS.length; j++) {
    
      if ( ID == dpNumS[j][0]  &&  dpNamS[i][0] == dpNumS[j][1] ){
  ...
  console.log(`${name} works in the ${dep} Department`);

}

for (let d = 0; d < 40; d++) {
  console.log(`---------------#${d+1}`);
  emSearch(bus,eID40[d]);
  console.log('---------------');
}
...
// console.log('\n',bus[0][1][1],'\n\n----0----\n');
// console.log(bus[1][0][1],'\n-----1----\n');
// console.log(bus[2][0][0],'\n------2-----\n');
// console.log(bus[2][0][2],bus[2][0][3],'\n------3-----\n');
// console.log(bus[3][0][1],'\n-----4---\n');

// function funk(array,num) {
...
//     console.log(`${i} ${array[2][i][2]} ${array[2][i][3]} eID ${array[2][i][0]} dID ${array[0][j][0]} Dep ${array[0][j][1]}`);
...
//   // }
//   // for (let i = 0; i < k60.length; i++) {
   
//   //   if(k35.indexOf(k60[i][0]) == -1){
     
//   //     k35.push([k60[i][0], k60[i][1]])
  
//   //   }
 
//   // }
 
//   // let cut = k35.slice(0,num)
...
//bus = [departments, employeeDepartment, employees,employeeSalaries]

function employeeSearch(array,ID) {

  let employeeSal =[],firstPay = 0, fn ='', ln = '',dep = '',depID = '',depNames = array[0], depNums = array[1], empNames = array[2], empSal = array[3]
  ...
  for (let i = 0; i < depNames.length; i++) {
    for (let j = 0; j < depNums.length; j++) {
     
      if (ID == depNums[j][0] && depNums[j][1] == depNames[i][0]){
  ...
  firstPay = employeeSal[0]

//⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇_______UNCOMMENT HERE FOR PART 2A_______⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇

  if (firstPay > 60000) {

    // console.log(`\nID#: ${ID}\nName: ${fn} ${ln}\nDepartment: ${depID} ${dep} \nsalary: $${firstPay.toLocaleString()}\n\n|-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-|`);
...
*/
     ...
     let highPay = employeeSal[employeeSal.length - 1]

//⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇_______UNCOMMENT HERE FOR PART 2B_______⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇

    // console.log(`\nID#: ${ID}\nName: ${fn} ${ln}\nDepartment: ${depID} ${dep} \nsalary: $${highPay.toLocaleString()}\n\n|-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-|`);
...
for (let i = 0; i < depNames.length; i++) {
  ...
  if (dpUp[1][1] == depNames[i][0] && dpUp.length > 1) {
  
    neW = depNames[i][1]

  }
  
  if (dpUp[0][1] == depNames[i][0] && dpUp.length > 1) {
...
//⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇_______UNCOMMENT HERE FOR PART 4_______⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇

if (dpUp.length > 1) {
  
  //console.log(`\nID# ${ID}: ${fn} ${ln} swapped from ${dpUp[0][1]}: ${old} Department to ${dpUp[1][1]}: ${neW} Department\n|-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-|`)
...
let startDate = '', lastDate = '', employed = ''

for (let i = 0; i < depNums.length; i++) {
  
  startDate = depNums[i][2]

  lastDate = depNums[i][3]

  //⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇_______UNCOMMENT HERE FOR PART 3_______⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇
  
  if (ID == depNums[i][0] && dpUp.length <= 1 && lastDate !== "9999-01-01") {
...
let raises = employeeSal.length

for (let i = 0; i < employeeSal.length; i++) {
 
  salSum += employeeSal[i]

}

//⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇_______UNCOMMENT HERE FOR PART 5_______⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇⬇

if ( raises > 1){
  
  //console.log(`ID# ${ID}: ${fn} ${ln} has recieved a raise ${employeeSal.length} times\n|-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-/-|\n`)
...
for (let i = 0; i < 35; i++) {
  ...
  employeeSearch(bus, eID40[i])

}

//employeeSearch(bus, eID40[0])
